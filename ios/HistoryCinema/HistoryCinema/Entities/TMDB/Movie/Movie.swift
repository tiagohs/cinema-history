...
struct Movie: BaseModel {
    ...
    func summary() -> String? {
        let defaultOverview = self.overview
        
        if let originalLanguage = self.originalLanguage {
            let translations = self.translations?.translations ?? []
            
            if let portugueseOverview = translations.first(where: {  $0.iso_639_1 == "pt" && $0.iso_3166_1 == "BR" })?.data?.overview, !portugueseOverview.isEmpty {
                return portugueseOverview
            }
            
            if let englishOverview = translations.first(where: { $0.iso_639_1 == "en" && $0.iso_3166_1 == "US" })?.data?.overview, !englishOverview.isEmpty {
                return englishOverview
            }
            
            if let originalOverview = translations.first(where: { $0.iso_639_1 == originalLanguage })?.data?.overview, !originalOverview.isEmpty {
    ...
    func trailerKey() -> String? {
        if let videos = self.videos?.results {
            return videos.first { $0.type == "Trailer" }?.key ?? videos.first?.key
        }
        
        return nil
    }
    
    func ranting() -> RantingDTO? {
        if let countries = self.releases?.countries {
            if let certificationBrazil = countries.first(where: { $0.country == "BR" })?.certification, !certificationBrazil.isEmpty {
                return RantingDTO(
                    country: "BR",
                    rating: certificationBrazil,
                    textColor: Color.white,
                    backgroundColor: MovieUtils.getBrazilRatingColor(certificate: certificationBrazil)
                )
            }
            
            if let certificationEua = countries.first(where: { $0.country == "US" })?.certification, !certificationEua.isEmpty {
                return RantingDTO(
                    country: "US",
                    rating: certificationEua,
                    textColor: Color.white,
    ...
    func externalLinks() -> [ExternalLinkDTO] {
        ...
        if let imdbId = self.externalIds?.imdbId, !imdbId.isEmpty {
            externalLinks.append(ExternalLinkDTO(image: "ic_imdb_grey", link: "https://www.imdb.com/title/\(imdbId)"))
        }
    ...
    static let exampleMovieExtraInfo = MovieExtraInfo(JSONString: "{\n        \"id\": 775,\n        \"title\": \"A Trip to the Moon\",\n        \"review_results\": [\n          {\n            \"language\": \"pt-BR\",\n            \"reviews\": [\n              {\n                \"reviewer\": \"plano_critico\",\n                \"reviewer_site_name\": \"Plano Crítico\",\n                \"date_formated\": \"3 de maio de 2015\",\n                \"reviewer_name\": \"Luiz Santiago\",\n                \"review_url\": \"https://www.planocritico.com/critica-viagem-a-lua-1902/\",\n                \"review_description\": \"Georges Méliès queria ter sido pintor, mas o pai não apoiou a ideia. Antes de se tornar o grande cineasta que foi, trabalhou na fábrica da família e construiu carreira nos palcos, como mágico ilusionista. Esteve presente no Salon Indien du Grand Café, em 28 de dezembro de 1895, quando os Irmãos Lumière exibiram seus primeiros curtas. Ofereceu dez mil francos aos Lumière pelas câmeras do cinematógrafo, oferta de pronto recusada pela dupla (que já havia recusado ofertas bem maiores do Museu de Cera de Paris – Musée Grévin – e até do Folies Bergère). Não podendo ter o cinematógrafo, Méliès foi a Londres e comprou um Teatógrafo (ou Animatógrafo) de Robert William Paul, eletricista e fabricante britânico de aparelhos ópticos, que também se tornou cineasta; máquina semelhante ao da famosa dupla francesa. E com este aparelho, o mágico, o inventor e o frustrado pintor em Méliès ganhou asas e criou alguns dos mais divertidos, aterrorizantes e fantásticos sonhos que o Primeiro Cinema conheceu.\",\n                \"review_rating\": 5.0\n              }\n            ]\n          },\n          {\n            \"language\": \"en-US\",\n            \"reviews\": [\n              {\n                \"reviewer_site_name\": \"1,001 Movies Reviewed Before You Die\",\n                \"date_formated\": \"21 de janeiro de 2019\",\n                \"reviewer_name\": \"Nathan Mitchell\",\n                \"review_url\": \"https://1001moviesreviewed.com/2019/01/21/re-review-a-trip-to-the-moon-1902/\",\n                \"review_description\": \"In this early silent film by director Georges Melies, we see a part of the start of cinema. An Astronomer (Victor Andre) studies the moon and builds a rocket. With his invention he assigns a Captain (Henri Delannoy) who leads his crew on a voyage to a mysterious land.\",\n                \"review_rating\": 5.0\n              }\n            ]\n          }\n        ],\n        \"watchOn\": [\n          {\n            \"type\": \"prime_video\",\n            \"link\": \"https://www.amazon.com/gp/video/detail/B07DPF8HJV\"\n          },\n          {\n            \"type\": \"youtube\",\n            \"link\": \"https://www.youtube.com/watch?v=xLVChRVfZ74\"\n          }\n        ],\n        \"did_you_know_list\": [\n          {\n            \"title\": \"O roubo que Méliès sofreu\",\n            \"description\": \"Depois de terminada as filmagens, Georges Méliès teve intenção de lançar o filme nos Estados Unidos e lucrar com ele. Infelizmente, o técnico Thomas A. Edison já tinha secretamente feito cópias, e em poucas semanas o filme já estava em terras americanas. Méliès nunca conseguiu nenhum dinheiro do sucesso que fez nos Estados Unidos, e faliu alguns anos mais tarde, enquanto Edison fez uma fortuna em cima do filme.\"\n          },\n          {\n            \"title\": \"Um filme pioneiro\",\n            \"description\": \"Esse foi um dos primeiros filmes que se tem conhecimento de ficção científica. Uma parte perto do final foi animada, fazendo com que esse seja um dos primeiros filmes animados também.\"\n          },\n          {\n            \"title\": \"Descoberta de uma cópia em 2002\",\n            \"description\": \"Em 2002, uma cópia do filme foi descoberto em um galpão na França. O incrível era que não só a maior parte do filme estava lá, mas que também ele era todo colorido à mão. O filme foi restaurado e lançado no ano seguinte no Pordenone Silent Festival.\"\n          }\n        ],\n        \"block_special\": {\n          \"type\": \"block_special\",\n          \"description\": \"Quando pensamos sobre <i>Viagem a lua</i>, nossa mente é logo tomada pela ideia inicial e mítica de que nos seus primórdios, cinema era uma arte cujas \\\"regras\\\" eram estabelecidas durante o próprio processo de produção. Este filme francês foi lançado em 1902 e representa uma revolução para a época, dada sua duração (aproximadamente 14 minutos) se comparado aos curtas-metragens de dois minutos que eram produzidos.<br/><br/> \\n\\nViagem à Lua reflete diretamente a personalidade teatral do seu diretor Georges Méliès, cujo passado de ator de teatro e mágico influenciou a produção do filme. A obra faz corajosas experiências com algumas das mais famosas técnicas cinematográficas, como superposições, fusões e práticas de montagem que seriam amplamente utilizadas no futuro. Apesar da simplicidade dos seus efeitos especiais, o filme é considerado o primeiro exemplo de cinema de ficção cientifica. Ele apresenta muitos elementos característicos do gênero - uma espaçonave, a descoberta de uma nova fronteira - e estabelece a maioria de suas convenções. <br/><br/> \\n\\nO filme de Méliès merece um lugar de destaque entre os ícones da história do cinema mundial. Apesar do seu estilo surreal, Viagem à Lua é divertido e inovador, conseguindo combinar os truques do teatro com as infinitas possibilidades da mídia cinematográfica. Méliès era mais um maestro do que um diretor, também participando como roteirista, ator, produtor, cenógrafo, figurinista e fotografo, criando efeitos especiais que foram considerados espetacular à época. Este primeiro filme de ficção científica é imperdível para aqueles interessados na origem das convenções que posteriormente influenciaram todo o gênero e seus mais famosos registros. \\n\\nDe modo mais geral, Viagem a Lua pode ser considerado o filme que estabelece a principal diferença entre ficção e não ficção cinematográfica. Em um tempo em que o cinema retrata, na maioria das vezes, a vida cotidiana como nos filmes dos irmãos <i>Lumière</i>, no final do século XIX, Méliès conseguiu oferecer uma fantasia que almejava o entretenimento puro e simples. Ele abriu a portas para os cineastas do futuro expressando visualmente sua criatividade de maneira completamente alheia aos filmes da época.\",\n          \"credits\": \"Análise original de <strong>Chiara Ferrari</strong>, adaptada do livro <i>1001 filmes para ver antes de morrer</i>\"\n        },\n        \"quote\": {\n          \"type\": \"quote\",\n          \"quote\": {\n            \"quote\": \"Méliès era verdadeiramente um magico... Ele compreendeu as possibilidades de uma câmera cinematográfica.\",\n            \"author\": \"Martin Scorsese\"\n          }\n        }\n      }")!
}
